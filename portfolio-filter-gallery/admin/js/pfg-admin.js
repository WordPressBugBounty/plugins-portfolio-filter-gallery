...
jQuery(document).ready(function ($) {
                ...
                success: function (response) {
                    if (response.success) {
                        $row.fadeOut(200, function () {
                            ...
                            // Update limit notice and form fields
                            var $notice = $('#pfg-filter-limit-notice');
                            var $formInputs = $('#pfg-add-filter-form').find('input, select, button');
    ...
    $('#pfg-add-filter-form').on('submit', function (e) {
        ...
        var count = $('.pfg-filter-row').length;
        if (count >= 6) {
            alert('Filter creation limit reached. You can create a maximum of 6 filters in the free version.');
            return false;
        }
